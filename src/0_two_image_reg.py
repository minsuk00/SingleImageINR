...
def simulate_sparse_ct(image_tensor, num_views, noise_intensity=None, randomize_angles=False):
    ...
    if randomize_angles:
        # 예: 0~180도 사이에서 랜덤한 시작점(Phase shift)을 줌
        offset = np.random.uniform(0, 180.0 / num_views)
        theta = np.linspace(0.0 + offset, 180.0 + offset, num_views, endpoint=False)
    else:
...
class AugmentationManager:
    ...
    def _generate_single_sample(self):
        ...
        if self.cfg.get("use_sparse_view_simulation", True):
            ...
            moving_img_sparse = simulate_sparse_ct(warped_clean_img, self.cfg['views'], self.cfg['noise_i0'], self.cfg["randomize_angle"])
        else:
...
common_config = {
    ...
    "noise_i0": None, # Poisson noise intensity (None to disable) 10000 For low dose, 1000000 for full-dose
...
experiment_updates = [
    # --- Experiment 1: Elastic Aug, No Simulation, With Communicator ---
    {
        ...
        # "communicator_depth": 8, 
        # "communicator_num_heads": 12,
        # "communicator_depth": 2, 
        # "communicator_num_heads": 8,
