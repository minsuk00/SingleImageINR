...
class LossBundle:
    ...
    def perceptual_loss(self, pred_img, target_img):
        ...
        with torch.no_grad(): # Loss doesn't need to track gradients
            lp = self.lpips_model(pred, targ).mean()

        self.lpips_model.to('cpu') # Move back to CPU
    ...
    def total(self, pred_img, target_img):
        ...
        # --- 1. Calculate RAW, UNWEIGHTED losses ---
        ...
        # --- 2. Log raw losses and add WEIGHTED losses to total ---
        if isinstance(l_pix, torch.Tensor) and l_pix.item() > 0:
            loss_dict['pixel'] = l_pix.item()          # Log unweighted
            total_loss += l_pix * self.pixel_w     # Add weighted to total
        
        if isinstance(l_ssim, torch.Tensor) and l_ssim.item() > 0:
            loss_dict['ssim'] = l_ssim.item()          # Log unweighted
            total_loss += l_ssim * self.ssim_w     # Add weighted to total
        
        if isinstance(l_lpips, torch.Tensor) and l_lpips.item() > 0:
            loss_dict['lpips'] = l_lpips.item()         # Log unweighted
            total_loss += l_lpips * self.lpips_w   # Add weighted to total
        
        if isinstance(l_chess, torch.Tensor) and l_chess.item() > 0:
            loss_dict['chess'] = l_chess.item()         # Log unweighted
            total_loss += l_chess * self.chess_w   # Add weighted to total
        
        if isinstance(l_dino, torch.Tensor) and l_dino.item() > 0:
            loss_dict['dino'] = l_dino.item()          # Log unweighted
            total_loss += l_dino * self.dino_w     # Add weighted to total
