...
class JointTrainer:
    ...
    def visualize(self):
        ...
        im_gjac = axes[1,0].imshow(gt_jac, cmap='summer', vmin=-1.0, vmax=2.0)
        axes[1,0].set_title(f"GT Jacobian Det (Folds: {gt_fold_perc:.2f}%)")
        plt.colorbar(im_gjac, ax=axes[1,0])
        im_pjac = axes[1,1].imshow(pred_jac, cmap='summer', vmin=-1.0, vmax=2.0)
        axes[1,1].set_title(f"Pred Jacobian Det (Folds: {pred_fold_perc:.2f}%)")
        plt.colorbar(im_pjac, ax=axes[1,1])
        axes[1,2].imshow(gt_warped_grid, cmap='gray')
        axes[1,2].set_title("GT Space Distortion")
        axes[1,3].imshow(warped_grid, cmap='gray')
        axes[1,3].set_title("Space Distortion (Pred Grid)")
